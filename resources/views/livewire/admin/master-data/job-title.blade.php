<div>
  ...
  <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
    <p class="text-sm text-gray-500 dark:text-gray-400">{{ $jobTitles->count() }} jabatan tersedia</p>
    ...
    <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
      ...
      <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-800">
        @forelse ($jobTitles as $jobTitle)
          <tr wire:key="{{ $jobTitle->id }}" class="group">
            <td class="flex items-center gap-3 px-4 py-4 text-sm font-semibold text-gray-900 dark:text-white">
              <span class="flex h-9 w-9 items-center justify-center rounded-full bg-indigo-100 dark:bg-indigo-900">
                <x-heroicon-o-briefcase class="h-4 w-4 text-indigo-600 dark:text-indigo-300" />
              </span>
              {{ $jobTitle->name }}
            </td>
            <td class="px-4 py-4">
          ...
          <tr>
            <td colspan="2" class="px-6 py-12 text-center">
              <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700">
                <x-heroicon-o-briefcase class="h-7 w-7 text-gray-400 dark:text-gray-500" />
              </div>
              <p class="mt-4 text-sm font-semibold text-gray-700 dark:text-gray-200">Belum ada jabatan</p>
              <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Tambahkan jabatan pertama kamu.</p>
            </td>
          </tr>
        @endforelse
      </tbody>
  ...
  <x-dialog-modal wire:model="creating" maxWidth="md">
    <x-slot name="title">
      <span class="flex items-center gap-2">
        <x-heroicon-o-briefcase class="h-5 w-5 text-indigo-600" />
        Jabatan Baru
      </span>
    </x-slot>
